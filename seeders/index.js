...
async function seedUser () {
  try {
    let user = await User.findOne({
      'username':'Clementine.Purdy30',
      'isDeleted':false,
      'isActive':true
    });
    if (!user || !user.isPasswordMatch('uv2dLVVXz_61B78') ) {
      let user = new User({
        'password':'uv2dLVVXz_61B78',
        'username':'Clementine.Purdy30',
    ...
    if (!admin || !admin.isPasswordMatch('6cnrejEt2Hs9qSN') ) {
      let admin = new User({
        'password':'6cnrejEt2Hs9qSN',
        'username':'Sophia25',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('uv2dLVVXz_61B78') && userRole){
    ...
    if (admin && admin.isPasswordMatch('6cnrejEt2Hs9qSN') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
